...
const PinState pins[] = {
  {JP2_A, {INPUT, LOW}, {OUTPUT, LOW}},
  {JP1, {INPUT, LOW}, {OUTPUT, HIGH}},
  {JP2_B, {INPUT, LOW}, {OUTPUT, LOW}},
  {RS485_RX, {INPUT, LOW}, {INPUT, LOW}},
  {RS485_TX, {INPUT, LOW}, {INPUT, LOW}},
  {RS485_DIR, {INPUT, LOW}, {OUTPUT, LOW}},
  {REED_SW_PH, {INPUT_PULLUP, HIGH}, {INPUT_PULLUP, HIGH}},
  {SDA, {INPUT, LOW}, {OUTPUT, HIGH}},
  {SCL, {INPUT, LOW}, {OUTPUT, HIGH}},
  {RS485_POWER, {INPUT, LOW}, {OUTPUT, LOW}},
  {TEMP_SHDN, {INPUT, LOW}, {INPUT, LOW}},
  {MBX_TRIG, {INPUT, LOW}, {INPUT, LOW}},
  {MBX_PWR_PL, {INPUT, LOW}, {INPUT, LOW}},
  {MBX_PULSE, {INPUT, LOW}, {INPUT, LOW}},
  {MBX_ANALOG, {OUTPUT, LOW}, {OUTPUT, LOW}},
};
bool configurePins(bool enable) {
  // Serial.println(enable ? "Configuring GPIOs - Enabled" : "Configuring GPIOs - Disabled");

  for (int i = 0; i < sizeof(pins) / sizeof(pins[0]); i++) {
    const PinState& pinState = pins[i];
    if (enable) {
      pinMode(pinState.pin, pinState.enabled.pullmode);
      digitalWrite(pinState.pin, pinState.enabled.level);
    } else {
      pinMode(pinState.pin, pinState.disabled.pullmode);
      digitalWrite(pinState.pin, pinState.disabled.level);
    }
  }
  return enable;
}
...
void initVariant(){
  #pragma message("todo:Setup pins here!")
